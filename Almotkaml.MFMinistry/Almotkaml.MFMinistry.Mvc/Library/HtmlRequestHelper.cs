...
namespace Almotkaml.MFMinistry.Mvc.Library
{
    public static class HtmlRequestHelper
    {
        ...
        public static MvcHtmlString DatePickerFor<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, object htmlAttributes = null)
        {
            ...
            {
                htmlAttributes = new {@class = "form-control mdate", placeholder = "YYYY-MM-DD" };
            }
            else
            {
                string cs = htmlAttributes.ToString();
                if (cs.Contains("01")) cs.Replace("01", "02");
                object cs0 = new { @class = "form-control mdate", placeholder = "YYYY-MM-DD" };

                cs = cs.Replace("}" , ","+ cs0.ToString() );
                htmlAttributes = cs;
            }
            string name = body.ToString().Replace("{", "");
            name = name.Replace("}", "");
